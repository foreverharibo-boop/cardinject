        ...
        if (options?.method?.toUpperCase() === 'POST' && typeof options?.body === 'string') {
            try {
                ...
                if (noteContent && Array.isArray(body.messages)) {
                    ...
                    console.log('[CI][ANCHOR DEBUG] noteText:', noteText);
                    console.log('[CI][ANCHOR DEBUG] anchor:', anchor);
                    console.log('[CI][ANCHOR DEBUG] system messages:',
                        body.messages
                            .filter(m => m.role === 'system')
                            .map((m, idx) => ({ idx, preview: (typeof m.content === 'string' ? m.content : JSON.stringify(m.content)).slice(0, 120) }))
                    );
                    ...
                    if (!inserted) {
                        ...
                        for (let i = body.messages.length - 1; i >= 0; i--) {
                            ...
                            if (role === 'user' || role === 'assistant') {
                                chatCount++;
                                if (chatCount >= noteDepth) { insertAt = i; break; }
                            }
                        }
